...
reed: chord(ch0rds).mode('above')
  .layer(x => x.anchor('c3').voicing().s("gm_bandoneon").hpf(200).lpf(4000).gain(.25).room(.4),
    x => x.anchor(song.pickRestart(["f4", "a4", "f4"])).voicing().struct("x").clip(.8).s("gm_cello").att(.1).gain(.2).rel(.3).hpf(200).room(1))
...
guit: n("[~ 0 1 2 1 0]/2").chord(ch0rds).mode('root').anchor('c4').voicing().s("gm_electric_guitar_clean:2").release(.5).hpf(400).gain(.5)
...
]).as("note:penv").layer(x => x.s("triangle").vib(5).vibmod(.1).clip(.95).adsr([0, .1, .7, .05]).room(.2).hpf(400).lpf(1000).gain(.9),
  x => x.transpose(12).s("saw").gain(.3))
